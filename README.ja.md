...
デフォルトの変換形式は mp3 です。変更したい場合は `configs/config.json` 内の `target_format` の値を変更してください (ex: "wav", "aac")。
デフォルトの表示言語は英語です。変更したい場合は `configs/config.json` 内の `language` の値を変更してください (ex: "ja", "cn", "kr")。
